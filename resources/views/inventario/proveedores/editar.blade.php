@extends('layouts.master')

@section('titulo', 'Editar proveedor')

@section('contenido')
    <section class="contenedor-personalizado">
        <div class="row match-height">
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Editar proveedor</h4>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            @if (session('mensaje'))
                                <div class="alert alert-success">{{ session('mensaje') }}</div>
                            @endif
                            <form method="POST" action="{{ route('editProv', $idProv) }}">
                                @method('PUT')
                                @csrf
                                <div class="row">
                                    <div class="col-sm-6">
                                        <label for="razonSocial">Razón social:</label>
                                        <div class="form-group position-relative has-icon-left">
                                            <input type="text"
                                                class="form-control @error('razonSocial') is-invalid @enderror"
                                                placeholder="Razón social" id="razonSocial" name="razonSocial"
                                                value="{{ $proveedor->razon_social }}" autofocus>
                                            <div class="form-control-icon">
                                                <i class="bi bi-person"></i>
                                            </div>
                                        </div>
                                        @error('razonSocial')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                            </div>
                                        @enderror
                                    </div>
                                    <div class="col-sm-6">
                                        <label for="estado">Estado:</label>
                                        <fieldset class="form-group">
                                            <select class="form-select" id="estado" name="estado">
                                                <option value="1" @if ($proveedor->ban_estado == 1) selected @endif>Activo</option>
                                                <option value="2" @if ($proveedor->ban_estado == 2) selected @endif>Inactivo</option>
                                            </select>
                                        </fieldset>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-6">
                                        <label for="estado">Giro:</label>
                                        <fieldset class="form-group">
                                            <select class="form-select" id="giro" name="giro">
                                                @foreach ($giro as $item)
                                                    <option value="{{ $item->id_giro }}" data-secondary-text="{{ $item->no_giro }}"
                                                    @if ($item->id_giro == $proveedor->giro_categoria) selected @endif>
                                                        {{ $item->desc_giro }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </fieldset>
                                    </div>
                                    <div class="col-sm-6">
                                        <label for="creditoFiscal">Crédito fiscal:</label>
                                        <div class="form-group position-relative has-icon-left">
                                            <input type="text"
                                                class="form-control @error('creditoFiscal') is-invalid @enderror"
                                                placeholder="Crédito fiscal" id="creditoFiscal" name="creditoFiscal"
                                                value="{{ $proveedor->credito_fiscal }}">
                                            <div class="form-control-icon">
                                                <i class="bi bi-person"></i>
                                            </div>
                                        </div>
                                        @error('creditoFiscal')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                            </div>
                                        @enderror
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-6">
                                        <label for="telefono">Teléfono:</label>
                                        <div class="form-group position-relative has-icon-left">
                                            <input type="text"
                                                class="form-control @error('telefono') is-invalid @enderror"
                                                placeholder="Teléfono" id="telefono" name="telefono"
                                                value="{{ $proveedor->telefono }}">
                                            <div class="form-control-icon">
                                                <i class="bi bi-person"></i>
                                            </div>
                                        </div>
                                        @error('telefono')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                            </div>
                                        @enderror
                                    </div>
                                    <div class="col-sm-6">
                                        <label for="direccion" class="form-label">
                                            Dirección
                                        </label>
                                        <textarea class="form-control @error('direccion') is-invalid @enderror" id="direccion" name="direccion"
                                            rows="3">{{ $proveedor->direccion }}</textarea>
                                        @error('direccion')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                            </div>
                                        @enderror
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="form-check">
                                        <div class="checkbox">
                                            <input type="checkbox" id="oferente" name="oferente"
                                                class="form-check-input" @if ($proveedor->oferente == 1) checked @endif>
                                            <label for="oferente">Oferente</label>
                                        </div>
                                    </div>
                                    <div class="form-check">
                                        <div class="checkbox">
                                            <input type="checkbox" id="sumBienes" name="sumBienes"
                                                class="form-check-input"  @if ($proveedor->suministrante_bienes == 1) checked @endif>
                                            <label for="sumBienes">Suministrador de bienes</label>
                                        </div>
                                    </div>
                                    <div class="form-check">
                                        <div class="checkbox">
                                            <input type="checkbox" id="presServ" name="presServ"
                                                class="form-check-input" @if ($proveedor->prestador_servicios == 1) checked @endif>
                                            <label for="presServ">Prestador de servicios</label>
                                        </div>
                                    </div>
                                    <div class="form-check">
                                        <div class="checkbox">
                                            <input type="checkbox" id="contratista" name="contratista"
                                                class="form-check-input" @if ($proveedor->contratista == 1) checked @endif>
                                            <label for="contratista">Contratista</label>
                                        </div>
                                    </div>
                                </div>
                                <button type="submit" class="btn btn-primary mt-3">
                                    <i class="fa-solid fa-user-pen me-2"></i>
                                    Actualizar
                                </button>
                                <a href="{{ route('operacion', 9) }}" class="btn btn-success mt-3 ms-3">
                                    <i class="fa-solid fa-left-long me-2"></i>
                                    Volver
                                </a>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
@endsection
